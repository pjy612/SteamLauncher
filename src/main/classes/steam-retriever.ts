...
class SteamRetriever {
  private readonly window = appGetWindow();

  private readonly accountSteamWebApiKey: string = storage.get('account.steamWebApiKey');

  private readonly accountLanguage: string = storage.get('account.language');
  ...
  private console(content: AxiosError | Error | string, space = false) {
    ...
    } else if (orgAxios.isAxiosError(nContent) && typeof nContent.config.url !== 'undefined') {
  ...
  private async getAppAchievements() {
    ...
    if (
      ...
      for (const statData of statsData) {
        const { name, defaultvalue } = statData;
